...
void av_node_2::initalise_avn(int cell_type) {
	...
	if (cell_type == 16) { //AN cell
		filename = "Input/AnCell.txt";
		std::cout << "AN Cell...\n";
		//filename.append(all_files_suffix);
	}
	else if (cell_type == 19) {
		filename = "Input/NhCell.txt";
		std::cout << "Nh Cell...\n";
		//filename.append(all_files_suffix);
	}
	else if (SKF == true && cell_type == 1) {
		filename = "Input/NCell_SKF.txt";
		//filename.append(all_files_suffix);
	}
	else if (SKF == false && cell_type == 1) {
		filename = "NCell_CONTROL.txt";
		//filename.append(all_files_suffix);
	}
	else {
...
void av_node_2::export_cell(int version) {
	std::string filename{ "Output" + all_files_suffix + "/avn_export_" };
	filename.append(std::to_string(version));
	filename.append(all_files_suffix);
